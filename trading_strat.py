...
# Download historical daily data for a given ticker
data = yf.download('TSLA', start='2024-06-01', end='2024-06-4')

# Fill missing values
data.fillna(method='ffill', inplace=True)

# Define trading strategies (e.g., Simple Moving Average Crossover)
def moving_average_crossover(data, short_window=10, long_window=30):
    data['SMA_short'] = data['Close'].rolling(window=short_window).mean()
    data['SMA_long'] = data['Close'].rolling(window=long_window).mean()
    
    # Generate buy and sell signals based on the crossover
    data['Signal'] = 0
